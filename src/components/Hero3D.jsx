...
export default function Hero3D() {
  ...
  useEffect(() => {
    ...
    const matAccent = new THREE.MeshStandardMaterial({ color: 0xff6b3d, roughness: 0.45, metalness: 0.1, emissive: 0x331005 })
    ...
    function jointBall(radius, mat = matJoint) {
    ...
    puddleCanvas.width = puddleCanvas.height = 256
    const pctx = puddleCanvas.getContext('2d')
    const grad = pctx.createRadialGradient(128, 128, 10, 128, 128, 128)
    grad.addColorStop(0, 'rgba(255,107,61,0.5)')
    grad.addColorStop(1, 'rgba(255,107,61,0)')
    pctx.fillStyle = grad
    pctx.fillRect(0, 0, 256, 256)
    const puddle = new THREE.Mesh(
      new THREE.PlaneGeometry(2.8, 2.8),
      new THREE.MeshBasicMaterial({ map: new THREE.CanvasTexture(puddleCanvas), transparent: true, depthWrite: false, opacity: 0.35 })
    )
    ...
    const particleMat = new THREE.PointsMaterial({
      color: 0xf5f1e8, size: 0.018, transparent: true, opacity: 0.55, depthWrite: false, sizeAttenuation: true,
    })
